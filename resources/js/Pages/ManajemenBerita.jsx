...
export default function Manajemen({ berita = [] }) {
    ...
    const applyFilter = (type) => {
        router.get('/manajemen', { filter: type }, { 
            preserveState: true, 
            replace: true 
        });
        setShowFilter(false);
            ...
            <main className="flex-1 p-6">
                ...
                <div className="flex justify-between mb-8 items-center">
                    ...
                    <div className="flex gap-2 relative">
                        <Link 
                            href="/berita/tambah" 
                            className="bg-white border border-gray-200 text-gray-600 p-2 rounded-lg hover:bg-gray-50 transition flex items-center justify-center"
                        >
                            <Plus size={18} />
                        </Link>
                            ...
                            {showFilter && (
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50">
                                    <button 
                                        ...
                                        className="w-full text-left px-4 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-600"
                                    >
                                        Hari Ini
                                    </button>
                                    <button 
                                        onClick={() => applyFilter('terbaru')}
                                        className="w-full text-left px-4 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-600"
                                    >
                                        Terbaru
                                    </button>
                ...
                {berita.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-100">
                        <p className="text-gray-400">
                            Belum ada berita. Klik "+" untuk menambah.
                        </p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {berita.map((item) => (
                            <div 
                                key={item.id} 
                                className="bg-white p-4 rounded-[2rem] shadow-sm flex gap-4 border hover:shadow-md transition"
                            >
                                ...
                                <div className="flex flex-col justify-between flex-1">
                                    <div>
                                        ...
                                        <p className="text-emerald-500 text-[11px] font-bold uppercase">
                                            {item.kategori}
                                        </p>
